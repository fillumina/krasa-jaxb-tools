...
public class Processor {
    ...
    /**
     * A signed number with an optional decimal point (i.e. "1", "-100" or "-2\.5"). The point
     * must be escaped because a bare '.' in a regexp is a wildcard.
     */
    ...
    class TypeProcessor {
        ...
        private void processType(
            ...
            if (fieldHelper.isNumber() || fieldHelper.isString()) {
                if (fieldHelper.isNumber()) {
                    // @Pattern cannot be applied to numbers: a literal pattern becomes a fixed range
                    final BigDecimal literal = literalNumericPattern(facet.getMultiPatterns());
                    annotator.addDecimalMinAnnotationInclusive(literal);
                    annotator.addDecimalMaxAnnotationInclusive(literal);
                }
        ...
        /**
         * Converts a {@code xsd:pattern} into a number if it is made of a literal number only.
         *
         * @return the value of the only pattern if it is a literal number (i.e. "1" or "-2\.5"),
         *         null otherwise.
         */
        private BigDecimal literalNumericPattern(
                final Collection<? extends Collection<String>> multiPatterns) {
            if (multiPatterns.size() != 1) {
                return null;
            }
            final Collection<String> patterns = multiPatterns.iterator().next();
            if (patterns.size() != 1) {
                return null;
            }
            final String pattern = patterns.iterator().next();
            if (!LITERAL_NUMBER.matcher(pattern).matches()) {
                return null;
            }
            // the regexp escaped decimal point (\.) must be unescaped to be parsed as a number
            return new BigDecimal(pattern.replace("\\", ""));
        }
    }
